...
function RecruiterConvoHandler:runScreenHandlers(pConvTemplate, pPlayer, pNpc, selectedOption, pConvScreen)
	...
	elseif screenID == "confirm_bribe" and CreatureObject(pPlayer):hasSkill("combat_smuggler_underworld_04") and (CreatureObject(pPlayer):getCashCredits() >= 200000)
		...
		self:add200kBribeOption(pNpc, clonedConversation)

	elseif (screenID == "accepted_bribe_40k") and CreatureObject(pPlayer):hasSkill("combat_smuggler_underworld_04") and (CreatureObject(pPlayer):getCashCredits() >= 40000)
		...
		recruiterScreenplay:grantBribe(pNpc, pPlayer, 40000, 250)

	elseif (screenID == "accepted_bribe_200k") and CreatureObject(pPlayer):hasSkill("combat_smuggler_underworld_04") and (CreatureObject(pPlayer):getCashCredits() >= 200000)
		and (getFactionPointsCap(CreatureObject(pPlayer):getFactionRank()) >= PlayerObject(pGhost):getFactionStanding(recruiterScreenplay:getRecruiterFaction(pNpc)) + 1250) then
		recruiterScreenplay:grantBribe(pNpc, pPlayer, 200000, 1250)
...
function RecruiterConvoHandler:add200kBribeOption(pNpc, screen)
	local faction = recruiterScreenplay:getRecruiterFaction(pNpc)
	if (faction == "rebel") then
		screen:addOption("Would a 200000 Gift Be Acceptable?", "accepted_bribe_200k")
	elseif (faction == "imperial") then
		screen:addOption("I just happen to have a spare 200000 credits for you.", "accepted_bribe_200k")
	end
end
...
function RecruiterConvoHandler:updateScreenWithBribe(pPlayer, pNpc, pConvTemplate, pConvScreen, faction)
	...
	if (CreatureObject(pPlayer):hasSkill("combat_smuggler_underworld_04") and (CreatureObject(pPlayer):getCashCredits() >= 40000)
