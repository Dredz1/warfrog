...
int PlayerObjectImplementation::addExperience(const String& xpType, int xp, bool notifyClient) {
	...
	if (experienceList.contains(xpType)) {
		...
		if (xp <= 0 && xpType != "jedi_general" && xpType != "gcw_skill_xp") {
			removeExperience(xpType, notifyClient);
			return 0;
		}
	}
		// -10 million experience cap for Jedi experience loss
    if (xp < -4000000 && xpType == "jedi_general") {
			xp = -4000000;
		}
    // -100k experience cap for GCW experience loss
    else if (xp < 0 && xpType == "gcw_skill_xp") {
        xp = 0;
	}
...
void PlayerObjectImplementation::notifyOnline() {
	...
	*luaOnPlayerLoggedIn << playerCreature;
	...
	if (missionManager != nullptr && playerCreature->hasSkill("force_title_jedi_rank_03")) {
...
void PlayerObjectImplementation::doFieldFactionChange(int newStatus) {
	...
	// -Mindsoft added to prevent processing for overt 
	//if (newStatus == FactionStatus::OVERT)
		//return;
	...
	if (newStatus == FactionStatus::COVERT) {
		...
		// -Mindsoft commented out to remove sf input popup (due to player complaints)	
