...
void PetControlDeviceImplementation::callObject(CreatureObject* player) {
	...
	int callTimer = 15; // Mindsoft added for timer control

	if (player->getCurrentCamp() == NULL && player->getCityRegion() == NULL) {
		// -----Mindsoft added for reduced non-combat droid call timers-----
		if (pet->isDroidObject() && petType == PetManager::DROIDPET) {
			ManagedReference<DroidObject*> calledDroid = cast<DroidObject*>(pet.get());
			if(calledDroid !=NULL && !calledDroid->isCombatDroid()){
				callTimer = 3;
			}
		}
		//------------------------------------------------------------------
		...
		message.setDI(callTimer); //Mindsoft Updated to reflect callTimer value
		player->sendSystemMessage(message);

		player->addPendingTask("call_pet", callPet, callTimer * 1000); //Mindsoft added callTimer
